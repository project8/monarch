...
bool PrintChannelsReal( const M4Stream* aStream, uint32_t aDataFormat );

template< typename XDataType >
bool PrintChannelsComplex( const M4Stream* aStream, uint32_t aDataFormat );
...
int main( const int argc, const char** argv )
{
    ...
    try
    {
        ...
        {
            const M4StreamHeader& tStrHeader = tReadHeader->StreamHeaders().at( iStream );
            ...
            const M4Stream* tStream = tReadTest->GetStream( iStream );
            ...
            for( unsigned iRec = 0; iRec < tMaxRecords; ++iRec )
            {
                ...
                {
                    throw M4Exception() << "There was a problem reading a record from this stream";
                }
                switch( tStrHeader.GetDataFormat() )
                {
                    case sDigitizedUS:
                        ...
                        {
                            throw M4Exception() << "Problem printing channels (int)";
                        }
                        break;
                    case sDigitizedS:
                    ...
                    case sAnalog:
                        switch( tStream->GetSampleSize() )
                        {
                            case 1:
                                ...
                                {
                                    throw M4Exception() << "Problem printing channels (float)" ;
                                }
                                break;
                            case 2:
                                switch( tStream->GetDataTypeSize() )
                                {
                                    case 4:
                                        ...
                                        {
                                            throw M4Exception() << "Problem printing channels (float-complex)" ;
                                        }
                                        break;
                                    case 8:
                                        ...
                                        break;
                                    default:
                                        throw M4Exception() << "Invalid data type size: " << tStream->GetDataTypeSize();
                                }
                                break;
                            default:
                                throw M4Exception() << "Invalid sample size: " << tStream->GetSampleSize();
                        }
                        break;
                    default:
                        throw M4Exception() << "Invalid data format: "<<tStrHeader.GetDataFormat();
                }
            }
...
bool PrintChannelsReal( const M4Stream* aStream, uint32_t aDataFormat )
{
    ...
    {
        const M4DataReader< XDataType > tDataInterface( aStream->GetChannelRecord( iChan )->GetData(),
...
bool PrintChannelsComplex( const M4Stream* aStream, uint32_t aDataFormat )
{
    ...
    for( unsigned iChan = 0; iChan < aStream->GetNChannels(); ++iChan )
    {
        const M4ComplexDataReader< XDataType > tDataInterface( aStream->GetChannelRecord( iChan )->GetData(),
